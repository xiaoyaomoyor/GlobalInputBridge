...
Filtered keys broadcast no events and can no longer start actions (Started); actions that were already active stop receiving `Triggered`, but their `Completed` is still emitted so blueprint Started handlers are always closed out. State queries always reflect the physical keyboard by default (frame edges and modifier state too); polling-style consumers such as movement mirroring can simply tick the **Respect Event Filter** checkbox on `Is Global Key Down` / `Was Global Key Pressed This Frame` / `Get Pressed Global Keys`, after which filtered keys read as not pressed. `Was Global Key Released This Frame` and modifier state intentionally ignore the filter — closing signals always flow, mirroring the Completed semantics. `Is Global Key Event Suppressed` remains available as a general building block. `Clear Global Input Event Filter` restores full broadcasting.
